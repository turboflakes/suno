use subxt::{
    ...
    config::TransactionExtensions,
    error::ExtrinsicError,
    tx::{DefaultParams, Payload, Signer, TransactionProgress},
    Config, Metadata, OnlineClientAtBlock,
};
...
pub fn should_use_v5_transaction(metadata: &Metadata) -> bool {
    metadata
        .extrinsic()
...
/// extension that embeds the signature. Chains that haven't added such an extension yet (only
/// have the single baseline pipeline) reject a V5 submission with "the transaction extension did
/// not authorize any origin" — there's nothing in their extension set that can attach an origin
/// outside of the classic V4 envelope's own Address+Signature fields.
///
...
pub async fn sign_and_submit_then_watch<T, Call, S>(
    client_at_block: &OnlineClientAtBlock<T>,
    call: &Call,
    signer: &S,
    params: <T::TransactionExtensions as TransactionExtensions<T>>::Params,
) -> Result<TransactionProgress<T, OnlineClientAtBlockImpl<T>>, ExtrinsicError>
where
    T: Config,
    Call: Payload,
    S: Signer<T>,
{
    if should_use_v5_transaction(client_at_block.metadata_ref()) {
        ...
        submittable.submit_and_watch().await
    } else {
        client_at_block
            .transactions()
...
/// Same as [`sign_and_submit_then_watch`], but passes default parameters to construct the
/// "signed extra" and "additional" payloads needed by the transaction — mirroring
/// `TransactionsClient::sign_and_submit_then_watch_default`.
#[allow(clippy::result_large_err)]
pub async fn sign_and_submit_then_watch_default<T, Call, S>(
    client_at_block: &OnlineClientAtBlock<T>,
    call: &Call,
    signer: &S,
) -> Result<TransactionProgress<T, OnlineClientAtBlockImpl<T>>, ExtrinsicError>
